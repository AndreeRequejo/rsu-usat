...
new class extends Component {
    ...
    {
        if (in_array($property, ['start_date', 'end_date', 'staff_group_id', 'zone_id', 'shift_id', 'vehicle_id', 'driver_id', 'helper_ids', 'work_days'], true) || str_starts_with($property, 'helper_ids.')) {
            $this->markAvailabilityDirty();
        }
    }
        ...
        return Scheduling::query()
            ...
            ->when($this->search !== '', function ($query) {
                $query->where(function ($searchQuery) {
                    $searchQuery->whereHas('vehicle', fn($q) => $q->where('name', 'like', '%' . $this->search . '%')->orWhere('plate', 'like', '%' . $this->search . '%'))->orWhereHas('groupDetails.employee', fn($q) => $q->where('first_name', 'like', '%' . $this->search . '%')->orWhere('last_name', 'like', '%' . $this->search . '%'));
                });
            })
            ->orderBy('date')
            ->orderBy('shift_id')
    ...
    {
        return StaffGroup::with(['zone', 'shift', 'vehicle'])
            ->where('active', true)
            ->orderBy('name')
            ->get();
    }
    ...
    public function maxHelpers(): int
    {
        ...
        if (!$vehicle || !$vehicle->occupant_capacity) {
            return 0;
        }
    ...
    public function drivers()
    {
        ...
        return Employee::where('employee_type_id', $driverType->id)->where('active', true)->whereHas('contracts', fn($q) => $q->where('is_active', true))->orderBy('first_name')->get();
    }
    ...
    public function helpersList()
    {
        ...
        return Employee::where('employee_type_id', $helperType->id)->where('active', true)->whereHas('contracts', fn($q) => $q->where('is_active', true))->orderBy('first_name')->get();
    }
    ...
    public function histories()
    {
        ...
        return SchedulingHistory::with('user')->where('scheduling_id', $this->historyId)->latest()->get();
    }
    ...
    public function massiveHolidays()
    {
        ...
        return Holiday::where('is_active', true)->whereDate('date', '>=', $this->massive_start_date)->whereDate('date', '<=', $this->massive_end_date)->orderBy('date')->get();
    }
    ...
    {
        $holidayDates = $this->holidaysForDates($dates)->pluck('date')->map(fn($date) => $date->format('Y-m-d'))->all();

        return $dates->reject(fn($date) => in_array($date->format('Y-m-d'), $holidayDates, true))->values();
    }
    ...
    private function matchingStaffGroupId(): ?int
    {
        ...
        return StaffGroup::query()->where('zone_id', $this->zone_id)->where('shift_id', $this->shift_id)->where('vehicle_id', $this->vehicle_id)->where('driver_id', $this->driver_id)->get()->first(fn(StaffGroup $group) => $group->helpers->pluck('id')->values()->toArray() === $helperIds)?->id;
    }
    ...
    {
        $this->reset(['change_shift_id', 'change_vehicle_id', 'change_person_role', 'change_person_id', 'change_turn_reason_preset', 'change_vehicle_reason_preset', 'change_person_reason_preset', 'change_turn_reason', 'change_vehicle_reason', 'change_person_reason', 'registeredChanges', 'availabilityErrors', 'turnChangeFeedback', 'turnChangeFeedbackType', 'vehicleChangeFeedback', 'vehicleChangeFeedbackType', 'personChangeFeedback', 'personChangeFeedbackType']);
    }
    ...
    {
        $holidayDates = $this->massiveHolidays->map(fn(Holiday $holiday) => $holiday->date->format('Y-m-d'))->all();
    ...
    private function massiveDatesForGroup(array $groupData)
    {
        ...
        return $this->massiveRangeDates()->filter(fn($date) => in_array($date->dayOfWeekIso, $workDays, true))->reject(fn($date) => in_array($date->format('Y-m-d'), $this->massiveExcludedHolidayDates, true))->values();
    }
    ...
    private function massiveUncoveredDates(array $groupData)
    {
        ...
        return $this->massiveRangeDates()->reject(fn($date) => in_array($date->format('Y-m-d'), $this->massiveExcludedHolidayDates, true))->reject(fn($date) => in_array($date->dayOfWeekIso, $workDays, true))->values();
    }
        ...
        foreach ($this->massiveGroups as $groupId => $groupData) {
            ...
            foreach ($dates as $date) {
                ...
                foreach (['driver_id' => 'Conductor'] as $field => $label) {
                    ...
                    if (GroupDetail::where('employee_id', $employeeId)->whereHas('scheduling', fn($query) => $query->whereDate('date', $dateKey)->where('shift_id', $groupData['shift_id']))->exists()) {
                ...
                foreach ($groupData['helpers'] ?? [] as $i => $helperId) {
                    ...
                    if (GroupDetail::where('employee_id', $helperId)->whereHas('scheduling', fn($query) => $query->whereDate('date', $dateKey)->where('shift_id', $groupData['shift_id']))->exists()) {
                        $message = $label . ' ' . $this->employeeName($employee) . ': Ya programado en ' . $date->format('d/m/Y') . '.';
                        $errors[] = $message;
                        $roleErrors[$field][] = $message;
                    }
    ...
    private function employeeDateProblems(int $employeeId, $date): array
    {
        ...
        $hasVacation = Vacation::where('employee_id', $employeeId)->where('status', 'Aprobada')->whereDate('start_date', '<=', $dateString)->whereDate('end_date', '>=', $dateString)->exists();
    ...
    public function workDaysLabel(array $workDays): string
    {
        ...
        return collect($workDays)->map(fn($day) => $labels[(int) $day] ?? null)->filter()->implode(', ');
    }

    private function upsertRegisteredChange(array $change): void
    {
        $this->registeredChanges = collect($this->registeredChanges)->reject(fn(array $registered) => $registered['field'] === $change['field'])->push($change)->values()->all();
    }
    ...
    private function validateReprogrammingState(array $state): array
    {
        ...
        foreach ($dates as $date) {
            $vehicleConflict = Scheduling::whereDate('date', $date->format('Y-m-d'))->where('shift_id', $state['shift_id'])->where('vehicle_id', $state['vehicle_id'])->when($this->editingId, fn($query) => $query->where('id', '!=', $this->editingId))->exists();
            ...
            foreach ($selectedEmployees as $employeeId) {
                $employeeConflict = GroupDetail::where('employee_id', $employeeId)
                    ->whereHas('scheduling', function ($query) use ($date, $state) {
                        $query->whereDate('date', $date->format('Y-m-d'))->where('shift_id', $state['shift_id'])->when($this->editingId, fn($q) => $q->where('id', '!=', $this->editingId));
                    })
                    ->exists();
                ...
                $hasVacation = Vacation::where('employee_id', $employeeId)->where('status', 'Aprobada')->whereDate('start_date', '<=', $date->format('Y-m-d'))->whereDate('end_date', '>=', $date->format('Y-m-d'))->exists();
    ...
    {
        $holidayDates = $dates->map(fn($date) => $date->format('Y-m-d'))->values()->all();
    ...
    {
        return $holidays->map(fn(Holiday $holiday) => $holiday->date->format('d/m/Y') . ' (' . $holiday->name . ')')->implode(', ');
    }
    ...
    private function validateContractsAndVacations($dates): void
    {
        foreach ($this->selectedEmployeeIds() as $employeeId) {
            ...
            $vacation = Vacation::where('employee_id', $employeeId)->where('status', 'Aprobada')->whereDate('start_date', '<=', $this->end_date)->whereDate('end_date', '>=', $this->start_date)->first();
    ...
    private function validateSchedulingConflicts($dates): void
    {
        ...
        foreach ($dates as $date) {
            $vehicleConflict = Scheduling::whereDate('date', $date->format('Y-m-d'))->where('shift_id', $this->shift_id)->where('vehicle_id', $this->vehicle_id)->when($this->editingId, fn($query) => $query->where('id', '!=', $this->editingId))->exists();
            ...
            foreach ($this->selectedEmployeeIds() as $employeeId) {
                $employeeConflict = GroupDetail::where('employee_id', $employeeId)
                    ->whereHas('scheduling', function ($query) use ($date) {
                        $query->whereDate('date', $date->format('Y-m-d'))->where('shift_id', $this->shift_id)->when($this->editingId, fn($q) => $q->where('id', '!=', $this->editingId));
                    })
                    ->exists();
    ...
    private function suggestReplacement(?Employee $employee): void
    {
        ...
        $replacement = Employee::where('employee_type_id', $employee->employee_type_id)->where('active', true)->where('id', '!=', $employee->id)->whereNotIn('id', $this->selectedEmployeeIds())->first();
    ...
    {
        $this->reset(['editingId', 'start_date', 'end_date', 'staff_group_id', 'zone_id', 'shift_id', 'vehicle_id', 'driver_id', 'helper_ids', 'work_days', 'notes', 'change_reason', 'availabilityChecked', 'availabilityValid', 'formChangedAfterValidation', 'availabilityErrors', 'availabilityWarnings', 'availabilitySuggestions']);
...
<div class="min-h-screen bg-white p-6 text-[#333333]">
    <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-[#333333] mt-1">
                {{ __('Gestion de programaciones operativas por grupo de personal.') }}</p>
        </div>

        <div class="flex flex-wrap gap-2">
            <flux:button :href="route('dashboard')" wire:navigate variant="primary" icon="home"
                class="bg-[#2E8B57]! text-white hover:bg-[#257046]!">
                {{ __('Ir al Dashboard') }}
            </flux:button>
            <flux:button wire:click="openCreate" variant="primary" icon="plus"
                class="bg-[#007bff]! text-white hover:bg-[#0069d9]!">
                {{ __('Nueva Programacion') }}
            </flux:button>
            <flux:button wire:click="openMassive" variant="danger" icon="queue-list"
                class="bg-[#E53935]! text-white hover:bg-[#C62828]!">
            ...
            <table class="w-full text-sm">
                ...
                <tbody>
                    @forelse ($this->schedulings as $i => $scheduling)
                        ...
                        @endphp
                        <tr wire:key="scheduling-{{ $scheduling->id }}"
                            class="{{ $scheduling->status === 'Finalizado' ? 'bg-green-50' : ($i % 2 === 0 ? 'bg-white' : 'bg-[#A5D6A7]/20') }} border-b border-[#A5D6A7] hover:bg-[#A5D6A7]/30 transition">
                            <td class="px-4 py-3 font-medium">{{ $scheduling->date?->format('d/m/Y') }}</td>
                            <td class="px-4 py-3">
                                <span
                                    class="inline-flex rounded-full px-3 py-1 text-xs font-bold {{ $scheduling->status === 'Finalizado' ? 'bg-green-100 text-green-700 border border-green-300' : ($scheduling->status === 'Reprogramado' ? 'bg-amber-100 text-amber-700 border border-amber-300' : 'bg-cyan-100 text-cyan-700 border border-cyan-300') }}">
                                    {{ $scheduling->status }}
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <span
                                    class="inline-flex rounded-full bg-[#F4C542]/30 px-3 py-1 text-xs font-semibold text-[#856404]">
                            ...
                            <td class="px-4 py-3">
                                {{ $driver ? trim($driver->first_name . ' ' . $driver->last_name) : '-' }}</td>
                            <td class="px-4 py-3">
                                <div class="space-y-1">
                                    @forelse ($helpers as $helper)
                                        <div class="rounded bg-gray-100 px-2 py-1 text-xs">
                                            {{ trim($helper->first_name . ' ' . $helper->last_name) }}</div>
                                    @empty
                                        <span>-</span>
                                    @endforelse
                                </div>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex justify-center gap-1">
                                    @if ($scheduling->status !== 'Finalizado')
                                        <button wire:click="openEdit({{ $scheduling->id }})"
                                            class="inline-flex h-8 w-8 items-center justify-center rounded-md bg-[#F4C542] text-white hover:bg-[#d9aa1f]"
                                            title="Modificar">
                                        ...
                                        <button wire:click="confirmDelete({{ $scheduling->id }})"
                                            class="inline-flex h-8 w-8 items-center justify-center rounded-md bg-[#E53935] text-white hover:bg-[#C62828]"
                                            title="Eliminar">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <button wire:click="finish({{ $scheduling->id }})"
                                            class="inline-flex h-8 w-8 items-center justify-center rounded-md bg-[#007bff] text-white hover:bg-[#0069d9]"
                                            title="Finalizar">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    @endif
                                    <button wire:click="openHistory({{ $scheduling->id }})"
                                        class="inline-flex h-8 w-8 items-center justify-center rounded-md bg-[#0ea5e9] text-white hover:bg-[#0284c7]"
                                        title="Historial">
    ...
    <flux:modal name="scheduling-form" wire:close="closeModal"
        class="w-[96vw]! md:w-[1320px]! max-w-none! max-h-[92vh] overflow-y-auto">
        <form wire:submit="save" class="space-y-5" novalidate>
            ...
            <div class="px-6 space-y-4">
                <div class="grid gap-4 md:grid-cols-[1fr_1fr_260px]">
                    ...
                    <div class="flex items-end">
                        <flux:button type="button" wire:click="validateAvailability" icon="check"
                            class="w-full bg-[#0ea5e9] text-white hover:bg-[#0284c7]">
                ...
                <flux:select wire:model.live="staff_group_id" label="Grupo de Personal *"
                    :disabled="filled($editingId)">
                    <option value="">{{ __('Seleccione un grupo') }}</option>
                    @foreach ($this->groups as $group)
                        <option value="{{ $group->id }}">{{ $group->name }} - {{ $group->zone?->name }} -
                            {{ $group->shift?->name }}</option>
                    @endforeach
                </flux:select>
                ...
                @if ($formChangedAfterValidation)
                    <div
                        class="rounded-md border border-cyan-400 bg-cyan-50 px-4 py-3 text-sm font-semibold text-cyan-700">
                ...
                @if ($availabilityChecked && !$formChangedAfterValidation && !empty($availabilityWarnings))
                    <div
                        class="max-h-40 overflow-y-auto rounded-md border border-amber-300 bg-amber-50 px-5 py-4 text-sm font-semibold leading-relaxed text-amber-800">
                ...
                @if ($availabilityChecked && !$formChangedAfterValidation && !$availabilityValid)
                    <div
                        class="max-h-48 overflow-y-auto rounded-md bg-[#E53935] px-5 py-4 text-sm font-semibold leading-relaxed text-white">
                ...
                <div class="grid gap-4 rounded-md border border-gray-200 bg-gray-50 p-4 md:grid-cols-4">
                    ...
                    <flux:select wire:model.live="shift_id" label="Turno"
                        :disabled="filled($staff_group_id) && !filled($editingId)">
                        <option value="">{{ __('Seleccione') }}</option>
                        @foreach ($this->shifts as $shift)
                            <option value="{{ $shift->id }}">{{ $shift->name }}
                                ({{ substr($shift->hour_in, 0, 5) }} - {{ substr($shift->hour_out, 0, 5) }})</option>
                        @endforeach
                    </flux:select>
                    <flux:select wire:model.live="vehicle_id" label="Vehiculo"
                        :disabled="filled($staff_group_id) && !filled($editingId)">
                        <option value="">{{ __('Seleccione') }}</option>
                ...
                @if ($maxHelpers > 0)
                    <div class="grid gap-4 md:grid-cols-{{ min($maxHelpers, 4) }}">
                        @for ($i = 0; $i < $maxHelpers; $i++)
                            <flux:select wire:model.live="helper_ids.{{ $i }}"
                                label="{{ __('Ayudante :num', ['num' => $i + 1]) }}"
                                :disabled="filled($staff_group_id) && !filled($editingId)">
                ...
                @elseif ($vehicle_id)
                    <div
                        class="flex items-center justify-center p-4 text-sm text-gray-400 border border-dashed border-[#A5D6A7] rounded-lg">
                ...
                <div>
                    <label
                        class="block text-sm font-semibold text-[#333333] mb-2">{{ __('Dias de trabajo *') }}</label>
                    <div class="flex flex-wrap gap-4 text-sm">
                        @foreach ([1 => 'Lunes', 2 => 'Martes', 3 => 'Miercoles', 4 => 'Jueves', 5 => 'Viernes', 6 => 'Sabado', 7 => 'Domingo'] as $day => $label)
                            <label class="inline-flex items-center gap-2">
                                <input type="checkbox" value="{{ $day }}" wire:model.live="work_days"
                                    disabled
                                    class="rounded border-gray-300 text-[#2E8B57] focus:ring-[#2E8B57] disabled:opacity-80">
                                <span>{{ $label }}</span>
                            </label>
                        @endforeach
                    </div>
                    @error('work_days')
                        <span class="mt-1 block text-xs text-[#E53935]">{{ $message }}</span>
                    @enderror
                </div>

                @if ($editingId)
                    <flux:textarea wire:model="change_reason" label="Motivo del cambio *" rows="3"
                        placeholder="Ingrese el motivo de la reprogramacion..." />
                @endif

                <flux:textarea wire:model="notes" label="Observaciones" rows="3"
                    placeholder="Observaciones adicionales..." />
            </div>

            <div class="flex justify-end gap-3 border-t border-gray-200 px-6 py-4">
                <flux:button type="button" variant="danger" wire:click="closeModal"
                    class="bg-[#E53935] text-white hover:bg-[#C62828]">
                    {{ __('Cancelar') }}
                </flux:button>
                <flux:button type="submit" variant="primary"
                    :disabled="!$availabilityChecked || !$availabilityValid || $formChangedAfterValidation"
                    class="bg-[#007bff] text-white hover:bg-[#0069d9] disabled:opacity-50">
                    {{ __('Guardar') }}
                </flux:button>
            </div>
        </form>
    ...
    <flux:modal name="massive-scheduling-form" wire:close="closeMassiveModal"
        class="w-[98vw]! md:w-[1400px]! max-w-none! max-h-[94vh] overflow-y-auto">
        <div class="space-y-5">
            ...
            <div class="px-5 space-y-5">
                <div class="grid gap-4 md:grid-cols-[1fr_1fr_360px]">
                    ...
                    <div class="flex items-end">
                        <flux:button type="button" wire:click="validateMassiveAvailability" icon="check-circle"
                            class="w-full border border-[#22c55e] bg-white text-[#22c55e] hover:bg-[#ecfdf5]">
                            {{ __('Validar Disponibilidad') }}
                        </flux:button>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button type="button" wire:click="setMassiveShiftFilter('')"
                            class="rounded-md border border-[#0ea5e9] px-3 py-2 text-sm font-semibold {{ $massive_shift_filter === '' ? 'bg-[#0ea5e9] text-white' : 'bg-white text-[#0ea5e9]' }}">
                            {{ __('Todos los Turnos') }}
                        </button>
                        @foreach ($this->shifts as $shift)
                            <button type="button" wire:click="setMassiveShiftFilter('{{ $shift->id }}')"
                                class="rounded-md border border-[#0ea5e9] px-3 py-2 text-sm font-semibold {{ (string) $massive_shift_filter === (string) $shift->id ? 'bg-[#0ea5e9] text-white' : 'bg-white text-[#0ea5e9]' }}">
                                {{ $shift->name }}
                            </button>
                        @endforeach
                    </div>
                ...
                <div>
                    ...
                    <div class="rounded-md border border-gray-300 bg-gray-100 p-4">
                        <div class="rounded bg-white px-3 py-2 text-sm font-bold text-[#075985]">
                            ...
                            <span
                                class="ml-2 text-xs font-normal text-gray-500">{{ __('Seleccione los que NO desea programar') }}</span>
                        </div>
                        <div class="mt-3 min-h-20 rounded border border-gray-200 bg-white p-3 text-sm">
                            @forelse ($this->massiveHolidays as $holiday)
                                <label class="mb-2 flex items-center gap-2">
                                    <input type="checkbox" value="{{ $holiday->date->format('Y-m-d') }}"
                                        wire:model.live="massiveExcludedHolidayDates"
                                        class="rounded border-gray-300 text-[#0ea5e9] focus:ring-[#0ea5e9]">
                                    <span>{{ $holiday->date->format('d/m/Y') }} - {{ $holiday->name }}</span>
                                </label>
                            @empty
                                <div class="text-gray-500">
                                    {{ __('Seleccione un rango de fechas para ver los feriados') }}</div>
                            @endforelse
                        </div>
                ...
                <div class="border-t pt-4">
                    ...
                    <div class="grid gap-4 xl:grid-cols-3">
                        @forelse ($massiveGroups as $groupId => $group)
                            @php
                                $result = $massiveValidation[$groupId] ?? [
                                    'errors' => [],
                                    'warnings' => [],
                                    'role_errors' => [],
                                    'role_warnings' => [],
                            ...
                            @endphp
                            <div
                                class="rounded-md border {{ $hasErrors ? 'border-red-300 bg-red-50' : 'border-gray-200 bg-gray-50' }} p-4">
                                <div class="mb-3 flex items-start justify-between gap-3">
                                    <div class="text-sm font-bold uppercase">{{ $group['name'] }}</div>
                                    <button type="button" wire:click="removeMassiveGroup({{ $groupId }})"
                                        class="inline-flex h-8 w-8 items-center justify-center rounded-md border border-red-400 text-red-500 hover:bg-red-50"
                                        title="Quitar">
                                        x
                                    </button>
                                </div>
                                ...
                                <div class="space-y-2 text-sm">
                                    ...
                                    <div><span class="font-bold">Turno:</span> <span
                                            class="rounded bg-[#0ea5e9] px-2 py-1 text-xs font-bold text-white">{{ $group['shift_name'] }}</span>
                                    </div>
                                    <div><span class="font-bold">Dias:</span>
                                        {{ $this->workDaysLabel($group['work_days']) }}</div>
                                    <div><span class="font-bold">Vehiculo:</span> <span
                                            class="rounded bg-[#0ea5e9] px-2 py-1 text-xs font-bold text-white">{{ $group['vehicle_label'] }}{{ $group['vehicle_capacity'] ? ' (Capacidad: ' . $group['vehicle_capacity'] . ')' : '' }}</span>
                                    </div>
                                </div>
                                ...
                                <div class="mt-4 space-y-3">
                                    <div>
                                        ...
                                        <select wire:model.live="massiveGroups.{{ $groupId }}.driver_id"
                                            class="w-full rounded-md border-gray-300 text-sm {{ !empty($result['role_errors']['driver_id'] ?? []) ? 'border-red-400 bg-red-100' : '' }}">
                                            <option value="">{{ __('Seleccione') }}</option>
                                            @foreach ($this->drivers as $employee)
                                                <option value="{{ $employee->id }}">{{ $employee->first_name }}
                                                    {{ $employee->last_name }}</option>
                                        ...
                                        @if (!empty($result['role_errors']['driver_id'] ?? []))
                                            <div
                                                class="mt-1 rounded bg-cyan-100 px-2 py-2 text-xs font-semibold text-cyan-900">
                                    ...
                                    @php
                                        $maxHelperSlots = $group['vehicle_capacity']
                                            ? max(0, $group['vehicle_capacity'] - 1)
                                            : 0;
                                    ...
                                    @for ($h = 0; $h < $helperCount; $h++)
                                        ...
                                        <div>
                                            <label
                                                class="mb-1 block text-sm font-bold">{{ __('Ayudante :num', ['num' => $h + 1]) }}:</label>
                                            <select
                                                wire:model.live="massiveGroups.{{ $groupId }}.helpers.{{ $h }}"
                                                class="w-full rounded-md border-gray-300 text-sm {{ !empty($result['role_errors'][$helperField] ?? []) ? 'border-red-400 bg-red-100' : '' }}">
                                                <option value="">{{ __('Seleccione') }}</option>
                                                @foreach ($this->helpersList as $employee)
                                                    <option value="{{ $employee->id }}">{{ $employee->first_name }}
                                                        {{ $employee->last_name }}</option>
                        ...
                        @empty
                            <div
                                class="rounded-md border border-gray-200 p-8 text-center text-sm text-gray-500 xl:col-span-3">
                ...
                @if ($massiveValidated)
                    <div class="space-y-3 pt-2">
                        <h3 class="text-base font-bold text-[#E53935]">{{ __('Resultado de Validacion General') }}
                        </h3>
                        @foreach ($massiveValidation as $groupId => $result)
                            ...
                            @if ($group)
                                <div class="overflow-hidden rounded-md border border-[#bfdbfe]">
                                    <div
                                        class="flex items-center justify-between bg-[#e0f2fe] px-4 py-3 text-sm font-semibold">
                                        <span>{{ $group['name'] }} - {{ $group['zone_name'] }} -
                                            {{ $group['shift_name'] }}</span>
                                        <span class="flex gap-2">
                                            @if (!empty($result['errors']))
                                                <span
                                                    class="rounded bg-[#E53935] px-2 py-1 text-xs text-white">{{ __('Con Errores') }}</span>
                                            @endif
                                            @if (!empty($result['warnings']))
                                                <span
                                                    class="rounded bg-[#facc15] px-2 py-1 text-xs text-[#333333]">{{ __('Con Advertencias') }}</span>
                                            @endif
                                        </span>
                                    </div>
                                    <div
                                        class="{{ !empty($result['errors']) ? 'bg-red-100 text-red-600' : 'bg-green-50 text-green-700' }} px-5 py-4 text-sm font-semibold">
                                        @if (!empty($result['warnings']))
            ...
            <div class="flex justify-center gap-3 border-t border-gray-200 px-5 py-4">
                <flux:button type="button" variant="danger" wire:click="closeMassiveModal"
                    class="bg-[#E53935] text-white hover:bg-[#C62828]">
                    {{ __('Cancelar') }}
                </flux:button>
                <flux:button type="button" wire:click="saveMassiveScheduling" icon="calendar-days"
                    class="bg-[#60a5fa] text-white hover:bg-[#3b82f6]">
                    {{ __('Guardar') }}
                </flux:button>
            </div>
        </div>
    ...
    <flux:modal name="reprogramming-form" wire:close="closeReprogrammingModal"
        class="w-[96vw]! md:w-[1180px]! max-w-none! max-h-[92vh] overflow-y-auto">
        <div class="space-y-5">
            <div class="bg-[#075985] px-6 py-4 text-white">
                <div class="flex items-center justify-between">
                    <flux:heading size="lg" class="text-white">{{ __('Modificar Programacion') }}
                    </flux:heading>
                </div>
            </div>
            ...
            <div class="grid gap-4 px-5 md:grid-cols-3">
                <div class="rounded-md border border-gray-200 bg-white shadow-sm">
                    ...
                    <div class="space-y-3 p-4">
                        ...
                        <flux:select wire:model="change_shift_id" label="Nuevo Turno">
                            ...
                            @foreach ($this->shifts as $shift)
                                <option value="{{ $shift->id }}">{{ $shift->name }}
                                    ({{ substr($shift->hour_in, 0, 5) }} - {{ substr($shift->hour_out, 0, 5) }})
                                </option>
                            @endforeach
                        </flux:select>
                        ...
                        <flux:textarea wire:model="change_turn_reason" label="Motivo del cambio" rows="2"
                            placeholder="Ingrese el motivo del cambio de turno" />
                        <flux:button type="button" wire:click="addTurnChange" icon="plus"
                            class="bg-[#0ea5e9] text-white hover:bg-[#0284c7]">
                            {{ __('Agregar cambio') }}
                        </flux:button>
                        @if ($turnChangeFeedback)
                            <div
                                class="rounded-md px-4 py-3 text-sm font-bold {{ $turnChangeFeedbackType === 'success' ? 'bg-[#28a745] text-white' : 'bg-[#E53935] text-white' }}">
                                {{ $turnChangeFeedback }}
                            </div>
                        @endif
                    </div>
                ...
                <div class="rounded-md border border-gray-200 bg-white shadow-sm">
                    ...
                    <div class="space-y-3 p-4">
                        ...
                        <flux:textarea wire:model="change_vehicle_reason" label="Motivo del cambio" rows="2"
                            placeholder="Ingrese el motivo del cambio de vehiculo" />
                        <flux:button type="button" wire:click="addVehicleChange" icon="plus"
                            class="bg-[#0ea5e9] text-white hover:bg-[#0284c7]">
                            {{ __('Agregar cambio') }}
                        </flux:button>
                        @if ($vehicleChangeFeedback)
                            <div
                                class="rounded-md px-4 py-3 text-sm font-bold {{ $vehicleChangeFeedbackType === 'success' ? 'bg-[#28a745] text-white' : 'bg-[#E53935] text-white' }}">
                ...
                <div class="rounded-md border border-gray-200 bg-white shadow-sm">
                    ...
                    <div class="space-y-3 p-4">
                        <flux:select wire:model.live="change_person_role" label="Personal actual">
                            ...
                            <option value="driver_id">{{ __('Conductor') }} -
                                {{ $this->employeeName(App\Models\Employee::find($driver_id)) }}</option>
                            @for ($h = 0; $h < $this->maxHelpers; $h++)
                                ...
                                <option value="helper_ids.{{ $h }}">
                                    {{ __('Ayudante :num', ['num' => $h + 1]) }} -
                                    {{ $helperPerson ? $this->employeeName($helperPerson) : __('Vacio') }}</option>
                            @endfor
                        </flux:select>
                        <flux:select wire:model="change_person_id" label="Nuevo personal">
                            ...
                            @php
                                $personnelList =
                                    $change_person_role === 'driver_id' ? $this->drivers : $this->helpersList;
                            @endphp
                        ...
                        <flux:textarea wire:model="change_person_reason" label="Motivo del cambio" rows="2"
                            placeholder="Ingrese el motivo del cambio de personal" />
                        <flux:button type="button" wire:click="addPersonChange" icon="plus"
                            class="bg-[#0ea5e9] text-white hover:bg-[#0284c7]">
                            {{ __('Agregar cambio') }}
                        </flux:button>
                        @if ($personChangeFeedback)
                            <div
                                class="rounded-md px-4 py-3 text-sm font-bold {{ $personChangeFeedbackType === 'success' ? 'bg-[#28a745] text-white' : 'bg-[#E53935] text-white' }}">
                            ...
                            @forelse ($registeredChanges as $index => $change)
                                <tr>
                                    ...
                                    <td class="border px-3 py-3 text-center">
                                        <button type="button"
                                            wire:click="removeRegisteredChange({{ $index }})"
                                            class="inline-flex h-8 w-8 items-center justify-center rounded-md bg-[#E53935] text-white hover:bg-[#C62828]"
                                            title="Quitar">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
            ...
            <div class="flex justify-end gap-3 border-t border-gray-200 px-5 py-4">
                <flux:button type="button" variant="danger" wire:click="closeReprogrammingModal"
                    class="bg-[#E53935] text-white hover:bg-[#C62828]">
                    {{ __('Cancelar') }}
                </flux:button>
                <flux:button type="button" wire:click="applyReprogramming" variant="primary"
                    class="bg-[#0ea5e9] text-white hover:bg-[#0284c7]">
                    {{ __('Guardar') }}
                </flux:button>
            </div>
    ...
    <flux:modal name="confirm-delete" class="md:w-100">
        <div class="space-y-5">
            ...
            <div class="flex justify-end gap-3">
                <flux:button x-on:click="Flux.modal('confirm-delete').close()" type="button">{{ __('Cancelar') }}
                </flux:button>
                <flux:button wire:click="delete" variant="danger" class="bg-[#E53935] text-white hover:bg-[#C62828]">
    ...
    <flux:modal name="history-modal" class="w-[94vw]! md:w-[960px]! max-w-none! max-h-[92vh] overflow-y-auto">
        <div class="space-y-5">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            @php
                                $historyEmployees =
                                    $this->historyScheduling?->groupDetails->pluck('employee')->filter()->values() ??
                                    collect();
                            @endphp
                            @foreach ($historyEmployees as $index => $employee)
                                <tr class="border-t">
                                    <td class="px-5 py-4">
                                        <span
                                            class="inline-flex rounded-md px-3 py-2 text-xs font-bold text-white {{ $index === 0 ? 'bg-[#0ea5e9]' : 'bg-[#22c55e]' }}">
                                ...
                                <tr class="border-t align-top">
                                    <td class="px-5 py-5 font-semibold text-[#075985]">
                                        ...
                                        <div class="text-xs font-normal text-gray-500">
                                            {{ $this->historyTime($history) }}</div>
                                    </td>
                                    <td class="px-5 py-5">
                                        <span
                                            class="inline-flex rounded-full px-3 py-1 text-xs font-bold {{ $this->historyTypeClass($history) }}">
            ...
            <div class="flex justify-center">
                <flux:button x-on:click="Flux.modal('history-modal').close()" type="button" variant="danger"
                    class="bg-[#E53935] text-white hover:bg-[#C62828]">
                    {{ __('Cerrar') }}
                </flux:button>
            </div>
