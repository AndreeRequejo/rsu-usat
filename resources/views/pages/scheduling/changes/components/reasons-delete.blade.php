<flux:modal name="confirm-delete-reason" class="md:w-100">
    <div class="space-y-5">
        <div class="flex items-start gap-4 px-6 pt-4">
            ...
            <div>
                ...
                <flux:text class="mt-1 text-sm text-[#666666]">
                    {{ __('Esta seguro de que desea eliminar este motivo? Esta accion no se puede deshacer.') }}
                </flux:text>
            </div>
        </div>
        <div class="px-6 py-4 bg-[#F5F5F5] border-t border-[#E0E0E0] flex justify-end gap-3">
            <flux:button x-on:click="Flux.modal('confirm-delete-reason').close()" type="button" variant="ghost" class="text-[#333333]">
                {{ __('Cancelar') }}
            </flux:button>
