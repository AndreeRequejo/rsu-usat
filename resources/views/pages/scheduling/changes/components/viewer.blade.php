            ...
            <div class="flex items-center gap-2 text-white">
                ...
                <div>
                    <flux:heading size="lg" class="text-white">
                        {{ __('Detalles del Cambio') }} #{{ $this->viewingData->id }}
                    </flux:heading>
                    <div class="text-sm text-white/80 flex items-center gap-2">
                        ...
                        {{ $this->viewingData->created_at->format('d/m/Y H:i') }}
                    </div>
                </div>
            </div>
            <div class="flex flex-col items-end gap-1">
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium text-white" style="background-color: {{ $this->viewingData->badge_color }}">
                    {{ $this->viewingData->change_label }}
                </span>
                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-white/20 text-white">
                    {{ $this->viewingData->type_label }}
                </span>
            </div>
        </div>

        <div class="overflow-y-auto px-6 py-5 space-y-4 max-h-[calc(95vh-140px)]">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-2">
                        <div class="flex items-center gap-2 text-sm">
                            <svg class="h-4 w-4 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                @if ($this->viewingData->change_type === 'turn')
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                                @elseif ($this->viewingData->change_type === 'vehicle')
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25" />
                                @else
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
                                @endif
                            </svg>
                            <span class="text-[#666666]">{{ strtoupper($this->viewingData->change_label) }}</span>
                        </div>
                        <div class="text-red-700 font-semibold">{{ $this->viewingData->old_value }}</div>
                    </div>
                </div>
                ...
                <div class="bg-green-50 rounded-xl border border-green-200 p-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="text-green-700 font-semibold">{{ $this->viewingData->new_value }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-blue-50 rounded-xl border border-blue-200 p-4">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="text-center">
                        ...
                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium text-white mt-1" style="background-color: {{ $this->viewingData->badge_color }}">
                            {{ $this->viewingData->change_label }}
                        </span>
                    </div>
                    <div class="text-center">
                        ...
                        <div class="text-sm font-semibold text-[#333333] mt-1">{{ $this->viewingData->user?->name ?? '-' }}</div>
                    </div>
                    <div class="text-center">
                        <div class="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center mx-auto mb-2">
                        ...
                        <div class="text-sm font-semibold text-[#333333] mt-1">{{ $this->viewingData->created_at->format('d/m/Y H:i') }}</div>
                    </div>
                    <div class="text-center">
                        <div class="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center mx-auto mb-2">
                        ...
                        <div class="text-xs text-[#666666] uppercase">{{ __('Programaciones') }}</div>
                        <div class="text-sm font-semibold text-[#333333] mt-1">#{{ $this->viewingData->affected_count }}</div>
                    </div>
                </div>
                ...
                <div class="mt-4 bg-white rounded-lg p-3 border border-blue-100">
                    ...
                    <p class="text-sm text-[#333333]">{{ $this->viewingData->reason ?? '-' }}</p>
                </div>
            </div>

            {{-- Programaciones afectadas --}}
            @if ($this->viewingData->type === 'massive' && $this->viewingChange->items->count() > 0)
                <div>
            ...
            @elseif ($this->viewingData->type === 'individual' && $this->viewingChange->scheduling)
                <div>
                    <h4 class="text-sm font-bold text-[#333333] mb-2">{{ __('Programacion afectada') }}</h4>
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm border border-[#A5D6A7] rounded-lg overflow-hidden">
                            <thead class="bg-[#2E8B57] text-white">
                                <tr>
                            ...
                            <tbody>
                                <tr class="border-b border-[#A5D6A7]">
                                    <td class="px-3 py-2">{{ $this->viewingChange->scheduling?->date?->format('d/m/Y') ?? '-' }}</td>
                                    <td class="px-3 py-2">{{ $this->viewingChange->scheduling?->zone?->name ?? '-' }}</td>
                                    <td class="px-3 py-2">{{ $this->viewingChange->scheduling?->shift?->name ?? '-' }}</td>
                                    <td class="px-3 py-2">{{ $this->viewingChange->scheduling?->vehicle?->name ?? '-' }}</td>
                                    <td class="px-3 py-2">{{ $this->viewingChange->scheduling?->groupDetails?->map(fn($gd) => ($gd->employee?->first_name ?? '') . ' ' . ($gd->employee?->last_name ?? ''))->implode(', ') ?? '-' }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
