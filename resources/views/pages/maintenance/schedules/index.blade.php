<div class="min-h-screen bg-white p-6 text-[#333333]">
    <div class="flex items-start justify-between mb-6">
        <div>
            <h1 class="text-3xl font-bold text-[#2E8B57]">
                {{ __('MANTENIMIENTO') }} {{ strtoupper($this->maintenance->name) }}
            </h1>
            <p class="text-sm text-[#333333] mt-1">
                {{ __('Horarios programados del ') . $this->maintenance->start_date->format('d/m/Y') . __(' al ') . $this->maintenance->end_date->format('d/m/Y') }}
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
            <div>
                ...
                <input
                    ...
                    placeholder="{{ __('Nombre del vehículo...') }}"
    ...
    <div class="bg-white rounded-xl shadow-sm border border-[#A5D6A7] overflow-hidden">
        @include('pages.maintenance.schedules.components.table')
    </div>

    @include('pages.maintenance.schedules.components.form')
    @include('pages.maintenance.schedules.components.delete')
    @include('pages.maintenance.schedules.components.detail')
</div>
